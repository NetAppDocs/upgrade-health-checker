---
sidebar: sidebar
permalink: set-up-upgrade-health-checker-multiple-clusters.html
keywords: upgrade, health, checker, set up, download
summary: "You can download Upgrade Health Checker to generate onsite, comprehensive reports before upgrading to a newer version of ONTAP."
---
= Download and set up Upgrade Health Checker for multiple clusters
:hardbreaks:
:icons: font
:linkattrs:
:imagesdir: ./media/
...
a| *VM* | m5.large VM or equivalent with 2 vCPUs and 8 GiB RAM.
 | Dedicated VM for UHC
a| *OS* | 
Any Linux OS with glibc library version 2.28 or higher for optimal compatibility. This includes:
* Ubuntu 22.04
* RHEL8
* RHEL9 | -
a| *Root Volume* | 100 GB minimum | Ensure 8+GB free in `/tmp`.
a| *NFS Volume* | An additional 100 GB minimum | For UHC run output - to ensure data preservation if the virtual machine is compromised.
a| *UHC install path* | `/opt/uhc/tool/` | Binary plus `config.yaml` file.
a| *Runs output path* | `/mnt/nfs/uhc/runs/` | On NFS volume for durability.
|===


//* Processing and memory: m5.large VM or equivalent with 2 vCPUs and 8 GiB RAM.
//* Recommended operating system: Any Linux OS with glibc library version 2.28 or higher for optimal compatibility. This includes:
//** Ubuntu 22.04
//** RHEL8
//** RHEL9
...
* Additionally, place the VM in a location with cluster connectivity and internet access to allow for automatic tool updates. The machine should have access to the following endpoints through HTTPS (443) to conduct automatic updates and to enable NetApp to receive AutoSupport information about your upgrade plans:
...
//Additionally, ensure the virtual machine can connect to the telemetry endpoint (https://support.netapp.com/ and https://api.activeiq.netapp.com) through HTTPS to enable NetApp to receive AutoSupport information about your upgrade plans.


.Steps
. Create the directory structure, mount the NFS volume to the virtual machine, and set up the Upgrade Health Checker binary.
.. Create the directory structure:
+
`mkdir -p /opt/uhc/tool`
+
`mkdir -p /mnt/nfs/uhc/runs`

.. Mount the NFS volume to the virtual machine:
+
`sudo mount -t nfs nfs-server:/uhc_data /mnt/nfs/uhc`

.. Set up the UHC binary and make it executable:
+
`sudo cp uhc /opt/uhc/tool/`
+
`sudo chmod +x /opt/uhc/tool/uhc`

. Create the configuration file `config.yaml` with the following content:
+
----
# opt/uhc/tool/config.yaml
# Production config file for Upgrade Health Checker

# Accept EULA (required for non-interactive environments)
ACCEPT_EULA: true

# Application configuration
APP:
  RUNS_PATH: "/mnt/nfs/uhc/runs/"
...
TARGET_ONTAP_VERSIONS: "" 
# leave empty to specify per-run or use "current" 

SKIP_COMPATIBILITY_CHECKS: false
----

CAUTION: For a production system, if you have an enterprise password management solution, for example, if you have CyberArk and Conjur, you can configure your environment to avoid passing credentials through a yaml file or command line. 

[start=3]
. [[step3]] Create the `uhctool` service account for each ONTAP cluster. 
+
Use the following CLI commands to configure the necessary permissions:
+
[source,cli]
----
security login rest-role create -role uhctool -api /api -access readonly -vserver <cluster-name>

security login rest-role create -role uhctool -api /api/support/autosupport -access read_create_modify -vserver <cluster-name>

security login rest-role create -role uhctool -api /api/support/autosupport/messages -access read_create_modify -vserver <cluster-name>
...
----
+
NOTE: You can build an Ansible playbook to automate the deployment of the `uhctool` role and service account across all ONTAP clusters.
+
After configuring each cluster, verify connectivity to each cluster:
+
`/opt/uhc/tool/uhc --test all --cluster-ip <cluster-mgmt-ip>`

.  Set up cron schedules or other automation to run Upgrade Health Checker. For example, you can set up a cron job to run Upgrade Health Checker weekly.
...
.. Set up the cron job to run the script weekly.
+
----
# Edit the crontab for the uhctool user
crontab -e

# Add the following line to schedule the script to run every Monday at 4:00 am
0 4 * * 1 /opt/uhc/tool/uhccron.sh
----

[NOTE]
================
* Use `--target-ontap-version current` for ongoing health monitoring without a specific upgrade target.
* Credentials are automatically read from `config.yaml`.
* EULA acceptance is set in `config.yaml`.
* Each cluster run create a separate output directory under `/mnt/nfs/uhc/runs/`
================

[start=5]
. [[step5]] On the day of your upgrade, run Upgrade Health Checker interactively for the target cluster to get the most current assessment before you begin your upgrade.
+
----
cd /opt/uhc/tool/
...
----
+
Review the output and address any issues before proceeding with your upgrade. If you have multiple clusters, run the above command for each cluster to get individual reports.



. Download the Upgrade Health Checker binary by link:https://console.netapp.com/automationHub[navigating to the NetApp Console Automation Hub] and finding the Upgrade Health Checker tile.
...
. (Optional) Set up credential management to secure access to the application. 
+
