...
[NOTE]
================
A static HTML dashboard is generated at `runs/dashboard/index.html` at the end of every Upgrade Health Checker run. The dashboard provides a high-level overview of the health status of all clusters that were assessed in that run, allowing you to easily monitor and compare the upgrade readiness of multiple clusters in one place. You can open the dashboard in a web browser to view the health status, potential blockers, and risks for each cluster at a glance.

----
./uhc --dashboard
----

This command aligns with `--runs-path / APP.RUNS_PATH` and `--config-path`, and exits after writing `runs/dashboard/index.html`. It does not require cluster credentials.
