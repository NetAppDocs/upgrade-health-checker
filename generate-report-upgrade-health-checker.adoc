...
[NOTE]
================ 
A static HTML dashboard is generated at `runs/dashboard/index.html` at the end of every Upgrade Health Checker run, using `--dashboard`. The dashboard provides a high-level overview of the health status of all the clusters that have been assessed recently, allowing you to easily monitor and compare the upgrade readiness of multiple clusters in one place. The dashboard aggregates runs from the last 30 days under `APP.RUNS_PATH`. You can open the `<runs>/dashboard/index.html` file directly in a browser to view the health status, potential blockers, and risks for each cluster at a glance; no web server or internet access is required.
