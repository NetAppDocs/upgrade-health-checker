...
You can automate also link:automate-upgrade-health-checker.html[automate generating upgrade reports] to assist with maintaining up-to-date upgrade plans if you have a large, complex environment with multiple clusters.

.Before you begin

To learn more about a specific Upgrade Health Checker parameter and the priority order for how the tool accepts input, refer to the link:upgrade-health-checker-input-parameters-guide.html[Input Parameters Guide for Upgrade Health Checker].
...
.. (Optional) You can skip Upgrade Health Checker's hardware compatibility checks when specifying the ONTAP version you want to upgrade to:
+
[source,cli]
----
...
----
+
You should only use this parameter if you are certain that your cluster hardware is compatible with the ONTAP version you want to upgrade to. Upgrade Health Checker enables hardware compatibility checks by default.
...
+
You can also specify the run path in the config.yaml file. If you do not specify a run path, the default path will be `./runs/<cluster-name>/`. The priority for specifying a run path is CLI parameter > config.yaml > default path.
