...
---
= Input parameters for Upgrade Health Checker
:hardbreaks:
:icons: font
:linkattrs:
:imagesdir: ./media/
...
The `--dashboard` parameter regenerates the dashboard from existing runs at `<runs>/dashboard/index.html` without contacting a cluster. You use it to refresh the fleet view outside of a normal run.  The dashboard provides a high-level overview of the health status of all the clusters that have been assessed recently, allowing you to easily monitor and compare the upgrade readiness of multiple clusters in one place. You can open the dashboard in a web browser to view the health status, potential blockers, and risks for each cluster at a glance.
